...
#define vThresh 650//threshold for IR detection was 2550   2613  2644    2675 2700
...
void __attribute__((interrupt, no_auto_psv)) _OC1Interrupt(void){
    ...
    if(state == 0){               //if the state is the initial state
        OC1count = 0;
        if(ADC1BUF15 > vThresh){ //&& ADC1BUF13 < vThresh2*0.5 && ADC1BUF14 < vThresh2*0.5 && ADC1BUF9 < vThresh2*0.5){  //And if the IR sensor toward the dispenser is greater than the IR threshold
    ...
    if (state == 3 && OC3R == servo_open){
         ...
         if (OC3count > 250){
            ...
            if (substate != 0){
                if(substate == 1){
                    OC2count = 0;
                    _LATA1 = 1;//set direction of aim motor
                    desiredSteps_aim = aim_steps;//sweep 90 degrees
                    OC2R = 1600;//turn on aiming PWM                                                    
                }
                else if(substate == 2){
                    OC2count = 0;
                    _LATA1 = 0;
...
//-----------------------------------------------------------
    ...
    OC1RS = 17000;             // Period of OC1 to achieve desired PWM 
...
//                        _LATA0 = 1; // START DC MOTORS
            ...
            if (IR_High == 0 && OC2count == 0 && substate == 1 && state == 3){//if front goal is active, and we are currently aimed at left goal
               ...
               // pollCount++; //__delay_ms(500);
                //if(pollCount > 000){
                    substate = 0;
                    __delay_ms(servo_wait/2);//give the motors time to speed up before opening the servo
                    OC3R = servo_open;
                //    pollCount = 0;
                //}
            }
